;;; Commentary:
;;
;; A simple operation-based CRDT that uses vector clocks and an
;; append-only event log.  Operations are processed in causal order.
;; Replicas query each other to obtain new events.  Those replicas
;; then initiate a pull of missing events.
