...
(define (chat id)
    ...
    (match msg
      ((id author cert-id created-at modified-at deleted-at contents reacts)
       (let ((name (or (assoc-ref names author) "?????"))
         ...
         (cond
          (deleted-at
           `(div (span ,name) ": --- deleted ---"))
          (else
           `(div (span ,name) ": " (span ,contents)
                 ;; (ul (li (button "edit"))
                 ;;     (li (button "delete")))
                 ;; ,@(match reacts
                 ;;     (() '())
  ...
  (define (switch-to-room room)
    (define (refresh-room-log)
      (let* ((actor (room-actor room))
             (names (*backend* 'send actor 'profiles)))
        (set-inner-shtml!
         (get-element-by-id "room-log")
         (map (lambda (msg) (render-message msg names))
              (*backend* 'send actor 'all-messages)))))
    (define (send-message)
  ...
  (set-inner-shtml!
   ...
   `(main (@ (class "chat-container"))
          ...
          (article (@ (id "context-header")))
          (article (@ (id "aux-pane")))
          (article (@ (id "main-pane")))))
  (refresh-room-list))
